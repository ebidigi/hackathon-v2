...
function doGet(e) {
  try {
    ...
    const results = TEAMS.map(name => ({
      ...
      voteCount: teamTotals[name].count,
      avgBusiness: teamTotals[name].count > 0 ? Math.round(teamTotals[name].business / teamTotals[name].count * 10) / 10 : 0,
      avgQuality: teamTotals[name].count > 0 ? Math.round(teamTotals[name].quality / teamTotals[name].count * 10) / 10 : 0,
      avgPres: teamTotals[name].count > 0 ? Math.round(teamTotals[name].pres / teamTotals[name].count * 10) / 10 : 0,
...
function updateSummary(ss) {
  ...
  // Header
  sheet.appendRow(['チーム名', '投票数', '業務活用度(合計)', 'クオリティ(合計)', 'プレゼン(合計)', '総合計', '順位']);
  ...
  sorted.forEach((t, i) => {
    sheet.appendRow([t.name, t.count, t.business, t.quality, t.pres, t.total, i + 1]);
  });
